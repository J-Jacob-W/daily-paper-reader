...
tldr: 针对EEG/MEG功能连接分析中如何定义超边的方法学问题，本文提出基于典型相干性（caCoh）构建超图的策略，将多元频谱连接作为超图的自然基础。通过控制仿真实验，caCoh超图相比基于幅度平方相干（MSC）的图在目标-基线对比上显著更高，且能恢复空间模式。该方法将每个频率的MSC边数从610减少到10或1个超边，验证了caCoh作为EEG/MEG超图构建方法的有效性。
source: arxiv
...
motivation: 超图可表示分布式神经交互，但缺乏从EEG/MEG频谱数据定义超边的系统方法，尤其是基于多元连接性的方式。
method: 提出两种caCoh超图：one-to-space和space-to-space，分别用外部信号或两个多维信号空间定义超边。
result: 仿真中caCoh超图在几乎所有信噪比下比MSC图有更高目标-基线对比，且将边数从610降至10或1。
conclusion: 多元频谱连接（caCoh）为EEG/MEG超图提供了天然且更优的方法学基础。
---

## 摘要
超图为表示分布在传感器集合上的神经生理交互提供了一个自然框架。一个关键的方法论问题是如何从频率分辨的脑电图/脑磁图（EEG/MEG）数据中定义超边。我们展示了一种构建策略，其中超边通过规范相干性（caCoh）获得，caCoh是相干性的扩展，用于估计多维信号空间之间的耦合。据我们所知，这是首个直接从专门为频率分辨神经生理分析设计的多元连接性度量构建超图的工作。我们提出了两种基于caCoh的表示：一对多空间超图，其中每个外部信号在EEG/MEG传感器空间上定义一个超边；以及空间到空间超图，其中两个多维信号空间由一个超边表示。我们在具有已知耦合频率和变化信噪比（SNR）的受控模拟中评估了该方法。与基于幅度平方相干性（MSC）的图相比，基于caCoh的超图在几乎所有SNR水平上显示出统计上更高的目标-基线对比度，表明更强地恢复了耦合频率。它们还恢复了与模拟源相关的传感器级空间模式。此外，一对多空间和空间到空间表示分别将每个频率的610个MSC边减少到10个和1个超边。这些结果确立了多元频谱连接性作为EEG/MEG超图的自然方法论基础。
