...
tldr: 血液学诊断中，单细胞形态与遗传数据（核型、突变）的整合至关重要。提出GenBloom框架，先对白细胞图像进行自监督预训练，再通过监督对比学习对齐遗传信息。该方法提升了下游诊断任务，超越组织病理学基础模型，并支持按疾病或遗传改变检索。
source: arxiv
...
motivation: 血液癌症诊断依赖单细胞形态与遗传数据，但现有模型未有效对齐多模态信息。
method: 两步训练：iBOT自监督预训练图像Transformer，再用监督对比学习对齐染色体畸变与基因突变。
result: 对齐的编码器在血液学诊断任务中优于组织病理学基础模型，并具备遗传检索能力。
conclusion: 整合遗传数据改善了患者表示，对齐临床流程，推动多模态血液学AI发展。
---

## 摘要
组织病理学编码器与转录组和基因组数据的多模态对齐已被证明能显著提升下游诊断任务的性能。血液细胞学独特之处在于，血液癌症诊断中通常将视觉单细胞评估与细胞遗传学和分子遗传学相结合。在本研究中，我们提出了一个框架，将单个白细胞图像与染色体畸变（核型）以及目标基因panel的体细胞突变进行对齐。我们的训练策略采用两阶段方法：（i）在超过1500名患者的队列上，使用iBOT头对转换器聚合器进行自监督、仅视觉的预训练；（ii）通过急性髓系白血病患者的监督对比损失进行遗传对齐。我们的遗传对齐患者编码器改善了血液学诊断任务，性能优于切片级组织病理学基础模型。此外，该模型为疾病和基因改变提供了即用型检索能力。将遗传数据纳入患者编码器提高了患者表示的质量，提供了一个与临床诊断流程对齐的框架，为未来的多模态血液学特定AI铺平了道路。代码和模型权重可在https://github.com/marrlab/GenBloom获取。
